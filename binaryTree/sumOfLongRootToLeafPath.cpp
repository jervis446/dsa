...
class Solution
{
    // we only need max Sum for len agar equal ho, else jyda length wale kaa sum
public:
    void helper(Node *root, int len, int &maxLen, int sum, int &maxSum) {
        //agar root last tak pahunch gya ho
        if(root == NULL) {
            //agar present length max length se badi ho gyi toh
            if(len > maxLen) {
                maxLen = len;
                maxSum = sum;
            }
